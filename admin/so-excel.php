...
if($nopo == ""){
    header('Location: export-so-excel.php');
    exit();
} else { 
    $querynopo = mysqli_query($conn, "SELECT * FROM so JOIN bom ON bom.bom_id = so.so_bom_id JOIN item ON item.item_id = bom.bom_item_id JOIN divisi ON divisi.divisi_id = bom.bom_divisi_id JOIN material ON material.material_id = bom.bom_material_id JOIN uom ON uom.uom_id = material.material_uom_id WHERE so.so_no_po = '$nopo'");

    if(mysqli_num_rows($querynopo) < 1){
    ...
    while($item = mysqli_fetch_assoc($querynopo)){
